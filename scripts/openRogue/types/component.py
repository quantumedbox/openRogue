...
def implement_component(obj: object, component: Component) -> object:
	...
	"""
	obj.__class__ = type("%s, %s" % (type(obj).__name__, component.__name__), (type(obj), component), {})
...
def deploy_back(obj: 'subclass of Component', attr: str, func: Callable, propf="setter") -> None:
	"""
	...
	if isinstance(a, property):
		...
		if propf == "getter":
			if not isinstance(a.fget, Pipe):
				setattr(obj.__class__, attr, property(Pipe(a.fget), a.fset, a.fdel))
			getattr(obj.__class__, attr).fget.push_back(func)

		elif propf == "setter":
			if not isinstance(a.fset, Pipe):
				setattr(obj.__class__, attr, property(a.fget, Pipe(a.fset), a.fdel))
			getattr(obj.__class__, attr).fset.push_back(func)

		elif propf == "deleter":
			if not isinstance(a.fdel, Pipe):
				setattr(obj.__class__, attr, property(a.fget, a.fset, Pipe(a.fdel)))
...
def deploy_front(obj: 'subclass of Component', attr: str, func: Callable, propf="setter") -> None:
	"""
	Helper for deploying Pipe components to object's function attribute
	Property arg is used to describe which property functions should be piped
	"""
